...
一个可配置的桌面导师 / 桌宠应用。主程序是 PySide6 写的透明置顶贴纸窗口，支持鼠标和触屏拖动、点击对话、空闲提醒、文件/文件夹拖放、OpenAI-compatible agent 接口，以及 Windows 打包。
...
- `assets/desktop_mentor.ico`：Windows exe 图标。
- `requirements.txt`：源码运行依赖。
...
已清理掉旧生成素材、预览图、临时 helper 和 Python bytecode 缓存；必要默认素材只保留 `assets/default_mentor.png` 和 `assets/desktop_mentor.ico`。
