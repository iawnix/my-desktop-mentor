...
- 电脑控制：路径检查、读取、列目录、搜索、打开、运行、创建/写入文件都走统一工具流程。
...
- 本地 skill 会按当前输入匹配并注入工作流提示；skill 只指导如何组合基础工具，不新增专用 function。
...
模型通过工具调用请求电脑操作时，读操作会直接执行并把工具结果回传给模型；打开、运行、创建和写入会先生成授权卡。点击 `允许本次` 后才会执行；点击 `拒绝` 不会动手。
设置里的 `Workspace` 是工具默认工作目录，也是模型创建任务文件的默认落点；应用安装目录只用于运行程序，不应作为计算或生成文件目录。

安全边界：

- 不支持删除文件。
- 只读工具直接执行；打开、运行、创建、写入都需要用户确认。
